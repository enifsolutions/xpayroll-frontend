...
const EMPTY: CrewForm = { name: '', code: '', description: '', leadId: '', departmentId: '', isActive: true };

export default function CrewsPage() {
  useRequirePermission(Permissions.Settings.Crews.View)
  const canManage = usePermission(Permissions.Settings.Crews.Manage)
  ...
  const load = async () => {
    ...
    } catch { showError('Load Failed', 'Could not load crews.') }
    finally { setLoading(false) }
  }

  useEffect(() => { if (initialized.current) return; initialized.current = true; load() }, [])

  const f = (field: keyof CrewForm) =>
  ...
  const handleSave = async () => {
    ...
    } catch (e: any) { setError(e?.response?.data?.message ?? 'Failed to save crew.') }
    finally { setSaving(false) }
  }

  const promptDelete = (item: Crew) => { setDeleteTarget(item); setConfirmOpen(true) }
  const handleDelete = async () => {
    if (!deleteTarget) return; setDeleting(true)
    try {
      await api.post('crews/save', { action: 'DELETE', id: deleteTarget.id, userId: 1 })
      setConfirmOpen(false); setDeleteTarget(null); await load()
      showSuccess('Crew Removed', deleteTarget.name)
    } catch { showError('Delete Failed', 'Could not remove crew.') }
  ...
  return (
    <>
      <div className="flex items-center justify-between mb-6">
        <div><h3 className="h3">Crews</h3><p className="text-gray-500 mt-1">Manage crew groups and their leads.</p></div>
        {canManage && <Button variant="solid" icon={<PlusIcon size={16} />} onClick={openAdd}>Add Crew</Button>}
      </div>
      <div className="card"><div className="card-body">
        {loading ? <div className="flex justify-center py-12"><div className="animate-spin h-6 w-6 border-2 border-primary border-t-transparent rounded-full" /></div> : (
          <table className="table-default table-hover w-full">
            ...
            <tbody>
              {items.length === 0 ? <tr><td colSpan={6} className="text-center py-8 text-gray-400">No crews found</td></tr>
                : items.map(item => (
                <tr key={item.id}>
        ...
        <div className="p-6 w-full max-w-lg">
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              <div><label className="form-label">Name <span className="text-red-500">*</span></label><Input value={form.name} onChange={f('name')} placeholder="Crew name" /></div>
              <div><label className="form-label">Code <span className="text-red-500">*</span></label><Input value={form.code} onChange={f('code')} placeholder="CRW001" /></div>
            </div>
            ...
            <div className="flex items-center gap-3">
              <input type="checkbox" id="crew_active" checked={form.isActive} onChange={e => setForm(p => ({ ...p, isActive: e.target.checked }))} className="w-4 h-4 rounded border-gray-300 text-primary" />
              <label htmlFor="crew_active" className="text-sm font-medium text-gray-700 dark:text-gray-300">Active</label>
            </div>
      ...
      <ConfirmDialog open={confirmOpen} variant="danger" title="Remove Crew"
