  ...
  const load = async () => {
    try {
      const [aRes, sRes, pRes] = await Promise.all([
        api.get(`shift-assignments/${employeeId}`),
        api.get('shift'),
        api.get('attendancepolicy'),
      ])
    ...
    <>
      ...
      {loading ? (
        ...
        <div className="text-center py-16 text-gray-400">
          No shift assignments yet. Click <strong>Assign Shift</strong> to get started.
        </div>
      ) : (
        <table className="table-default table-hover w-full">
          ...
          <tbody>
            {assignments.map((a) => (
              <tr key={a.id}>
                <td>
                  <span className="heading-text font-medium">{a.shiftName}</span>
                  <span className="ml-2 text-xs text-gray-400">{a.shiftCode}</span>
                </td>
                <td>{a.attendancePolicyName}</td>
                <td>{fmt(a.effectiveFrom)}</td>
                <td>{fmt(a.effectiveTo)}</td>
                <td>
                  <span className={`xp-badge ${a.isActive ? 'xp-badge-success' : 'xp-badge-neutral'}`}>
      ...
      <Dialog isOpen={dialogOpen} onClose={closeDialog} onRequestClose={closeDialog}>
        <div className="p-6 w-full max-w-md">
          <h5 className="h5 mb-4">{editing ? 'Edit Shift Assignment' : 'Assign Shift'}</h5>

          <div className="space-y-4">
            <div>
              <label className="form-label">Shift <span className="text-red-500">*</span></label>
              <select className="input w-full" value={form.shiftId} onChange={(e) => setForm({ ...form, shiftId: e.target.value })}>
            ...
            <div>
              <label className="form-label">Attendance Policy <span className="text-red-500">*</span></label>
              <select className="input w-full" value={form.attendancePolicyId} onChange={(e) => setForm({ ...form, attendancePolicyId: e.target.value })}>
            ...
            <div>
              <label className="form-label">
                Effective To
                <span className="text-gray-400 text-xs ml-1">(leave blank for open-ended)</span>
              </label>
