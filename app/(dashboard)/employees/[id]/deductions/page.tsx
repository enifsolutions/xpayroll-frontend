  ...
  const load = async () => {
    try {
      const [dRes, dtRes] = await Promise.all([
        api.get(`employee-deductions/${employeeId}`),
        api.get('deductiontype'),
      ])
    ...
    <>
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-0.5">Manage deduction assignments for this employee.</p>
        </div>
      ...
      {loading ? (
        ...
        <div className="text-center py-16 text-gray-400">
          No deductions assigned. Click <strong>Add Deduction</strong> to get started.
        </div>
      ) : (
        <table className="table-default table-hover w-full">
          ...
          <tbody>
            {deductions.map((d) => (
              <tr key={d.id}>
                <td>
                  <span className="heading-text font-medium">{d.deductionName}</span>
                  <span className="ml-2 text-xs text-gray-400">{d.deductionCode}</span>
                  {(d.overrideAmount != null || d.overridePercentage != null) && (
                    <span className="ml-2 xp-badge xp-badge-info text-xs">Override</span>
                  )}
                </td>
                <td>
                  <span className={`xp-badge ${TYPE_BADGE[d.deductionType] ?? 'xp-badge-neutral'}`}>
                    {d.deductionType}
                  </span>
                  {d.isStatutory && (
                    <span className="ml-1 xp-badge xp-badge-warning text-xs">Statutory</span>
                  )}
                </td>
                ...
                <td>
                  <span className={`xp-badge ${d.isActive ? 'xp-badge-success' : 'xp-badge-neutral'}`}>
      ...
      <Dialog isOpen={dialogOpen} onClose={closeDialog} onRequestClose={closeDialog}>
        <div className="p-6 w-full max-w-md">
          <h5 className="h5 mb-4">{editing ? 'Edit Deduction' : 'Add Deduction'}</h5>
          <div className="space-y-4">

            <div>
              <label className="form-label">Deduction Type <span className="text-red-500">*</span></label>
              <select
                className="input w-full"
                value={form.deductionDefinitionId}
                onChange={(e) => setForm({ ...form, deductionDefinitionId: e.target.value })}
                disabled={!!editing}
              >
            ...
            {selectedType && (
              <div className="bg-gray-50 dark:bg-gray-800 rounded-lg px-4 py-3 text-sm text-gray-500 space-y-1">
                ...
                {selectedType.isStatutory && (
                  <div className="flex justify-between">
                    <span>Statutory</span>
                    <span className="xp-badge xp-badge-warning text-xs">Yes</span>
                  </div>
                )}
              </div>
            )}
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                <label className="form-label">Effective To <span className="text-gray-400 text-xs">(optional)</span></label>
            ...
            {editing && (
              <div className="flex items-center justify-between">
                ...
                <Switcher checked={form.isActive} onChange={(val: boolean) => setForm({ ...form, isActive: val })} />
              </div>
            )}
      ...
      <ConfirmDialog
        ...
        message={deleteTarget ? `Are you sure you want to remove "${deleteTarget.deductionName}"? This action cannot be undone.` : ''}
