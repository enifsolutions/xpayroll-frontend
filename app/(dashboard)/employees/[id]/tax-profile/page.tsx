  ...
  const load = async (year = yearFilter) => {
    try {
      const [pRes, eRes, tcRes] = await Promise.all([
        api.get(`employee-tax-profiles/${employeeId}?year=${year}`),
        api.get(`employee-tax-profiles/${employeeId}/exemptions?year=${year}`),
        api.get('taxconfig'),
      ])
    ...
    <>
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-0.5">Tax configuration and exemptions for this employee.</p>
        </div>
        <div className="flex items-center gap-3">
          <select className="input" value={yearFilter} onChange={(e) => handleYearChange(parseInt(e.target.value))}>
        ...
        <div className="space-y-6">
          ...
          <div>
            <div className="flex items-center justify-between mb-3">
              <h6 className="font-semibold text-gray-700 dark:text-gray-300">Tax Configuration</h6>
              {!profile && (
                <Button variant="solid" icon={<PlusIcon size={15} />} onClick={openAddProfile}>
                  Set Tax Profile
                </Button>
              )}
            </div>
            ...
            {profile ? (
              <div className="border border-gray-200 dark:border-gray-700 rounded-xl p-5 space-y-4">
                <div className="flex items-start justify-between">
                  <div>
                    <p className="font-semibold heading-text">{profile.taxConfigName}</p>
                    <p className="text-sm text-gray-500 mt-0.5">{profile.regime} · Tax Year {profile.taxYear}</p>
                  </div>
                  <div className="flex gap-1">
                    <button onClick={() => openEditProfile(profile)} className="p-1.5 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-700 text-gray-500 hover:text-primary transition-colors"><Pencil size={15} /></button>
                    <button onClick={() => promptDelete(profile.id, profile.taxConfigName, 'profile')} className="p-1.5 rounded-lg hover:bg-red-50 dark:hover:bg-red-900/20 text-gray-500 hover:text-red-500 transition-colors"><Trash2 size={15} /></button>
                  </div>
                </div>

                <div className="grid grid-cols-3 gap-4">
                  <div className="bg-gray-50 dark:bg-gray-800 rounded-lg p-3">
                    <p className="text-xs text-gray-500 mb-1">Residency Status</p>
                    <p className="font-semibold heading-text">{profile.residencyStatus}</p>
                  </div>
                  <div className="bg-gray-50 dark:bg-gray-800 rounded-lg p-3">
                    <p className="text-xs text-gray-500 mb-1">Cumulative Tax Paid</p>
                    <p className="font-semibold heading-text">LKR {fmt(profile.cumulativeTaxPaid)}</p>
                  </div>
                  <div className="bg-gray-50 dark:bg-gray-800 rounded-lg p-3">
                    <p className="text-xs text-gray-500 mb-1">Cumulative Taxable Income</p>
                    <p className="font-semibold heading-text">LKR {fmt(profile.cumulativeTaxableIncome)}</p>
                  </div>
                </div>
              </div>
            ) : (
              <div className="text-center py-8 text-gray-400 border border-dashed border-gray-200 dark:border-gray-700 rounded-xl">
                No tax profile for {yearFilter}. Click <strong>Set Tax Profile</strong> to configure.
              </div>
            )}
          </div>
          ...
          <div>
            <div className="flex items-center justify-between mb-3">
              <div>
                <h6 className="font-semibold text-gray-700 dark:text-gray-300">Tax Exemptions</h6>
                {exemptions.length > 0 && (
                  <p className="text-xs text-gray-400 mt-0.5">Total: LKR {fmt(totalExemptions)} / year</p>
                )}
              </div>
              <Button variant="solid" icon={<PlusIcon size={15} />} onClick={openAddExemption}>
                Add Exemption
              </Button>
            </div>
            ...
            {exemptions.length === 0 ? (
              ...
              <table className="table-default table-hover w-full">
                ...
                <tbody>
                  {exemptions.map((ex) => (
                    <tr key={ex.id}>
                      <td><span className="heading-text font-medium">{ex.exemptionType}</span></td>
                      <td className="text-gray-500 text-sm">{ex.description ?? '—'}</td>
                      <td className="text-right font-medium">LKR {fmt(ex.annualAmount)}</td>
                      <td>
                        {ex.isApproved ? (
                          ...
                          <span className="xp-badge xp-badge-warning">Pending</span>
                        )}
                      </td>
                      <td>
                        <div className="flex justify-end gap-1">
                          <button onClick={() => openEditExemption(ex)} className="p-1.5 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-700 text-gray-500 hover:text-primary transition-colors"><Pencil size={15} /></button>
                          <button onClick={() => promptDelete(ex.id, ex.exemptionType, 'exemption')} className="p-1.5 rounded-lg hover:bg-red-50 dark:hover:bg-red-900/20 text-gray-500 hover:text-red-500 transition-colors"><Trash2 size={15} /></button>
                        </div>
                      </td>
                    </tr>
                  ))}
      ...
      {/* Tax Profile Dialog */}
      <Dialog isOpen={profileDialogOpen} onClose={() => setProfileDialogOpen(false)} onRequestClose={() => setProfileDialogOpen(false)}>
        <div className="p-6 w-full max-w-md">
          <h5 className="h5 mb-4">{editingProfile ? 'Edit Tax Profile' : 'Set Tax Profile'}</h5>
          <div className="space-y-4">
            <div>
              <label className="form-label">Tax Configuration <span className="text-red-500">*</span></label>
              <select className="input w-full" value={profileForm.taxConfigId}
                onChange={(e) => setProfileForm({ ...profileForm, taxConfigId: e.target.value })}
                disabled={!!editingProfile}>
                <option value="">Select tax config…</option>
                {taxConfigs.map((tc) => (
                  <option key={tc.id} value={tc.id}>{tc.name} · {tc.regime} ({tc.taxYear})</option>
                ))}
              </select>
            </div>
            <div>
              <label className="form-label">Residency Status</label>
              <select className="input w-full" value={profileForm.residencyStatus}
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <select className="input w-full" value={profileForm.year}
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <Input type="number" step="0.01" value={profileForm.cumulativeTaxPaid}
                  onChange={(e) => setProfileForm({ ...profileForm, cumulativeTaxPaid: e.target.value })} />
              </div>
              <div>
                <label className="form-label">Cumulative Taxable Income</label>
                <Input type="number" step="0.01" value={profileForm.cumulativeTaxableIncome}
                  onChange={(e) => setProfileForm({ ...profileForm, cumulativeTaxableIncome: e.target.value })} />
              </div>
            </div>
            <div className="flex justify-end gap-2 pt-2">
              <Button variant="plain" onClick={() => setProfileDialogOpen(false)} disabled={profileSaving}>Cancel</Button>
              <Button variant="solid" loading={profileSaving} onClick={handleProfileSave}>
      ...
      {/* Exemption Dialog */}
      <Dialog isOpen={exemptionDialogOpen} onClose={() => setExemptionDialogOpen(false)} onRequestClose={() => setExemptionDialogOpen(false)}>
        <div className="p-6 w-full max-w-md">
          <h5 className="h5 mb-4">{editingExemption ? 'Edit Exemption' : 'Add Exemption'}</h5>
          <div className="space-y-4">
            <div>
              <label className="form-label">Exemption Type <span className="text-red-500">*</span></label>
              <Input placeholder="e.g. Medical, EPF, Housing Loan" value={exemptionForm.exemptionType}
                onChange={(e) => setExemptionForm({ ...exemptionForm, exemptionType: e.target.value })} />
            </div>
            <div>
              <label className="form-label">Description</label>
              <Input placeholder="Optional description" value={exemptionForm.description}
                onChange={(e) => setExemptionForm({ ...exemptionForm, description: e.target.value })} />
            </div>
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="form-label">Annual Amount <span className="text-red-500">*</span></label>
                <Input type="number" step="0.01" value={exemptionForm.annualAmount}
                  onChange={(e) => setExemptionForm({ ...exemptionForm, annualAmount: e.target.value })} />
              </div>
              <div>
                <label className="form-label">Year</label>
                <select className="input w-full" value={exemptionForm.year}
                  onChange={(e) => setExemptionForm({ ...exemptionForm, year: e.target.value })}>
                  {Array.from({ length: 5 }, (_, i) => CURRENT_YEAR - 2 + i).map((y) => (
                    <option key={y} value={y}>{y}</option>
                  ))}
                </select>
            ...
            <div className="flex items-center justify-between">
              ...
              <Switcher checked={exemptionForm.isApproved}
                onChange={(val: boolean) => setExemptionForm({ ...exemptionForm, isApproved: val })} />
            </div>
            <div className="flex justify-end gap-2 pt-2">
              <Button variant="plain" onClick={() => setExemptionDialogOpen(false)} disabled={exemptionSaving}>Cancel</Button>
              <Button variant="solid" loading={exemptionSaving} onClick={handleExemptionSave}>
