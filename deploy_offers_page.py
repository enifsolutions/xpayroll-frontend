#!/usr/bin/env python3
"""
Deploys the unified Offers page (list + KPI stats + full lifecycle actions)
to the XpayRoll frontend, replacing the old Offer Approvals-only page.
...
FRONTEND_ROOT = "/Users/gayansaranga/My Files/XpayRoll/xpayroll-frontend"
TARGET_DIR = os.path.join(FRONTEND_ROOT, "app", "(dashboard)", "recruitment", "offers")
TARGET_FILE = os.path.join(TARGET_DIR, "page.tsx")
OLD_APPROVALS_FILE = os.path.join(TARGET_DIR, "approvals", "page.tsx")
...
export default function OffersPage() {
  ...
  const canConvert = usePermission(Permissions.Recruitment.Onboarding.ConvertToEmployee);
  ...
  const loadLookups = async () => {
    try {
      ...
      setBranches(
        (branchRes.data ?? []).map((b: any) => ({ id: String(b.id), name: b.name })),
      );
      setDesignations(
        (designationRes.data ?? []).map((d: any) => ({ id: String(d.id), name: d.title ?? d.name })),
      );
    } catch {
  ...
  async function handleApprove() {
    ...
    try {
      setApproving(true);
      await api.post("/offers/save", { id: approveTarget.id, action: "APPROVE" });
      setApproveTarget(null);
      await load();
      showSuccess("Approved", `Offer for ${approveTarget.candidateName} approved.`);
    } catch (err: any) {
      showError("Approve failed", err?.response?.data?.error ?? "Could not approve the offer.");
    } finally {
      setApproving(false);
    }
  }
  ...
  async function handleReject() {
    ...
    try {
      ...
      setRejectionReason("");
      await load();
      showSuccess("Rejected", `Offer for ${rejectTarget.candidateName} rejected.`);
    } catch (err: any) {
      showError("Reject failed", err?.response?.data?.error ?? "Could not reject the offer.");
    } finally {
      setRejecting(false);
    }
  }
  ...
  async function handleSubmitForApproval(item: OfferItem) {
    try {
      ...
      await load();
      showSuccess("Submitted", `Offer for ${item.candidateName} submitted for approval.`);
    } catch (err: any) {
      showError("Submit failed", err?.response?.data?.error ?? "Could not submit the offer.");
    } finally {
  ...
  async function handleSaveEdit() {
    ...
    try {
      ...
      showError("Save failed", err?.response?.data?.error ?? "Could not save the offer.");
    } finally {
      setSaving(false);
    }
  }
  ...
  async function handleSend() {
    ...
    try {
      ...
      showError("Send failed", err?.response?.data?.error ?? "Could not send the offer.");
    } finally {
      setSending(false);
    }
  }
  ...
  async function handleWithdraw() {
    ...
    try {
      setWithdrawing(true);
      await api.post("/offers/save", { id: withdrawTarget.id, action: "WITHDRAW" });
      setWithdrawTarget(null);
      await load();
      showSuccess("Withdrawn", `Offer for ${withdrawTarget.candidateName} withdrawn.`);
    } catch (err: any) {
      showError("Withdraw failed", err?.response?.data?.error ?? "Could not withdraw the offer.");
    } finally {
  ...
  async function handleAccept() {
    ...
    try {
      ...
      await load();
      showSuccess("Accepted", `Offer for ${acceptTarget.candidateName} marked as accepted.`);
    } catch (err: any) {
      showError("Update failed", err?.response?.data?.error ?? "Could not record acceptance.");
    } finally {
      setAccepting(false);
    }
  }
  ...
  async function handleDecline() {
    ...
    try {
      ...
      setDeclineReason("");
      await load();
      showSuccess("Declined", `Offer for ${declineTarget.candidateName} marked as declined.`);
    } catch (err: any) {
      showError("Update failed", err?.response?.data?.error ?? "Could not record decline.");
    } finally {
      setDeclining(false);
    }
  }
  ...
  async function handleDelete() {
    ...
    try {
      ...
      await load();
      showSuccess("Deleted", `Offer for ${deleteTarget.candidateName} deleted.`);
    } catch (err: any) {
      showError("Delete failed", err?.response?.data?.error ?? "Could not delete the offer.");
    } finally {
      setDeleting(false);
    }
  }
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h3 className="text-xl font-bold text-gray-900 dark:text-white">Offers</h3>
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-0.5">
            Every offer across every candidate — create, approve, send, and track in one place.
          </p>
        </div>
        <button
          onClick={load}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="card">
          <div className="card-body flex items-center gap-4">
            ...
            <div>
              <p className="text-xs text-gray-500 dark:text-gray-400">Rejected This Week</p>
              <p className="text-xl font-bold text-gray-900 dark:text-white">
                  ...
                  <tbody>
                    {items.map((item) => (
                      <tr key={item.id}>
                        <td>
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              <button
                                onClick={() =>
                                  router.push(`/recruitment/pipeline?applicationId=${item.applicationId}`)
                                }
                                className="font-semibold text-gray-900 dark:text-white text-sm text-left hover:text-violet-500"
                              >
                                {item.candidateName}
                              </button>
                              <p className="text-xs text-gray-400">{item.requisitionTitle}</p>
                            </div>
                          </div>
                        </td>
                        ...
                        <td className="text-sm">{item.salaryOffered.toLocaleString()}</td>
                        <td className="text-sm">
                          {item.startDate ? new Date(item.startDate).toLocaleDateString() : "—"}
                        </td>
                        <td>
                          <span className={`xp-badge ${STATUS_BADGE[item.status] ?? "xp-badge-neutral"}`}>
                        ...
                        <td>
                          <div className="flex items-center justify-end gap-1">
                            ...
                            {item.status === "Draft" && item.requiresApproval && canEdit && (
                              <button
                            ...
                            {item.status === "Accepted" && !item.convertedEmployeeId && canConvert && (
                              <button
                            ...
                            {item.status === "Accepted" && item.convertedEmployeeId && (
                              <span className="text-xs text-gray-400 px-1">Converted</span>
                            )}
                            {["Draft", "PendingApproval", "Approved", "Sent"].includes(item.status) &&
                              canWithdraw && (
                                <button
        ...
        <div className="space-y-4">
          <div>
            ...
            <Input
              ...
              onChange={(e: any) => setEditForm((f) => ({ ...f, salaryOffered: e.target.value }))}
            />
          </div>
          <div>
          ...
          <div>
            ...
            <Input
              ...
              onChange={(e: any) => setEditForm((f) => ({ ...f, startDate: e.target.value }))}
            />
          </div>
        </div>
        <div className="flex justify-end gap-2 mt-6">
          ...
          <Button variant="solid" color="primary" loading={saving} onClick={handleSaveEdit}>
            Save
          </Button>
        </div>
      </Dialog>
      ...
      >
        ...
        <Input type="date" value={expiryDate} onChange={(e: any) => setExpiryDate(e.target.value)} />
        <div className="flex justify-end gap-2 mt-6">
          <Button variant="default" onClick={() => setSendTarget(null)}>
            Cancel
          </Button>
          <Button variant="solid" color="primary" loading={sending} onClick={handleSend}>
            Send
          </Button>
        </div>
      </Dialog>
...
function ConvertToEmployeeDialog({
  ...
  useEffect(() => {
    ...
    api
      .get("/leave-templates")
      .then((res) =>
        setLeaveTemplates((res.data ?? []).map((x: any) => ({ id: String(x.id), name: x.name }))),
      )
  ...
  async function handleConvert() {
    ...
    try {
      ...
      showSuccess("Converted", `${offer.candidateName} is now an employee record.`);
      onConverted();
      onClose();
    } catch (err: any) {
      showError(
        "Conversion failed",
        err?.response?.data?.error ?? err?.response?.data?.message ?? "Could not convert to employee.",
      );
    } finally {
      setConverting(false);
    }
  ...
  return (
    <Dialog isOpen={true} onClose={onClose} onRequestClose={onClose} width={460}>
      <h5>Convert to Employee</h5>
      <p className="text-sm text-gray-500 dark:text-gray-400 mt-1 mb-4">
        Creates a new employee record for {offer.candidateName} from this accepted offer.
      </p>
      <div className="space-y-4">
        <div>
        ...
        <div>
          ...
          <select
            className="input w-full"
            value={status}
            onChange={(e) => setStatus(e.target.value as "Active" | "Probation")}
          >
      ...
      <div className="flex justify-end gap-2 mt-6">
        ...
        <Button variant="solid" color="primary" loading={converting} onClick={handleConvert}>
          Convert
        </Button>
      </div>
    </Dialog>
...
def main():
    ...
    if os.path.exists(OLD_APPROVALS_FILE):
        print(f"\nNOTE: old approvals page still exists at:\n  {OLD_APPROVALS_FILE}")
        print("It is now superseded by the new unified page above.")
        print("Delete it manually once you've verified the new page works:")
        print(f'  rm -r "{os.path.dirname(OLD_APPROVALS_FILE)}"')
    else:
        print("\nNo old approvals page found at the expected path — nothing to clean up.")
