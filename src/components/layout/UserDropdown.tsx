...
export function UserDropdown() {
  ...
  const initials = (n?: string) =>
    (n ?? 'AP').split(' ').map(w => w[0]).join('').slice(0, 2).toUpperCase()
  ...
  return (
    <div ref={ref} className="relative">
      ...
      {open && (
        <div className="absolute right-0 top-[calc(100%+8px)] w-[280px] z-50 rounded-xl
                        bg-[var(--xp-surface)] border border-[var(--xp-border)]
                        shadow-2xl shadow-black/20 overflow-hidden">

          {/* Profile header */}
          <div className="px-4 py-4 border-b border-[var(--xp-border)]"
               style={{ background: 'linear-gradient(135deg, rgba(29,78,216,0.08) 0%, rgba(109,40,217,0.08) 100%)' }}>
            <div className="flex items-center gap-3">
              <div className="w-11 h-11 rounded-full flex items-center justify-center text-white font-semibold flex-shrink-0 text-sm"
                   style={{ background: 'linear-gradient(135deg,#1d4ed8,#6d28d9)' }}>
                {initials(user?.fullName)}
              </div>
          ...
          {menuItems.map((group, gi) => (
            <div key={group.group} className={gi > 0 ? 'border-t border-[var(--xp-border)]' : ''}>
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="text-xs font-medium text-[var(--xp-text-1)]">{item.label}</div>
                      <div className="text-[10px] text-[var(--xp-text-2)]">{item.sub}</div>
                    </div>
                    <ChevronRight size={13} className="text-[var(--xp-text-3)] group-hover:text-[var(--xp-text-2)] transition-colors" />
                  </button>
                )
              })}
            </div>
          ...
          <div className="border-t border-[var(--xp-border)] p-2">
            ...
            >
              ...
              <div className="flex-1">
                ...
                <div className="text-[10px] text-[var(--xp-text-2)]">End your session</div>
              </div>
            </button>
          </div>
